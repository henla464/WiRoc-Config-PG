...
;(function($) {
	...
	$.peekABar = function(options) {
		...
		var init = function(jqObjToAddBarTo) {
			that.settings = $.extend({}, defaults, options);
			_create(jqObjToAddBarTo);
		...
		/** Create the Bar */
		var _create = function(jqObjToAddBarTo) {
			that.bar = $('<div></div>').addClass('peek-a-bar').attr('id', '__peek_a_bar_' + rand);
			jqObjToAddBarTo.prepend(that.bar);
			that.bar.hide();
		};
